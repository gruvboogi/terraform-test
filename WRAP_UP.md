...
### 3.2 Autonomous Database 버전 호환성 이슈
- **상황:** ADB 생성 시 `409 IncorrectState` 오류 반복 발생.
- **시도:**
    1.  `26ai` 버전 시도 → 실패 (테넌시 미지원).
    2.  `23ai` 버전 다운그레이드 → 실패 (동일 오류).
    3.  **전략 수정 (Pivot):** 컴퓨트 모델을 **OCPU**에서 **ECPU**로 변경하고 다시 `26ai` 시도.
- **해결:** ECPU 모델을 적용하자 최신 `26ai` 버전 생성이 성공함. (OCI의 최신 기능 제약 조건 확인 및 우회).
